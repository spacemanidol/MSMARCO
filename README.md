...
6. wellFormedAnswers. An array of rewritten answers, most contain a single answer but ~1% contain more than one answer(average of ~5 answers if there are multiple answers). These answers were generated by reading the previous answer and query combo rewriting into a well formed answer(full sentence, proper gramar, understandable without query context).
